...
export async function updateOrderStatus(formData: FormData) {
    ...
    const { data: order, error: fetchError } = await supabase
        .from('orders')
        .select('*, products(name), vendor_products:vendor_product_id(name), profiles:profiles!orders_buyer_id_fkey(phone_number)')
        .eq('id', orderId)
        .single();
    ...
    if (newStatus === 'completed' && oldStatus !== 'completed') {
        ...
        if (targetId) {
            ...
            await supabase.from(table).update({ quantity: newQuantity }).eq('id', targetId);
        }
    ...
    if (newStatus === 'ready' && oldStatus !== 'ready') {
        ...
        const productName = order.products?.name || order.vendor_products?.name;
