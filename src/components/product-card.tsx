...
export function ProductCard({ product, user, isSaved, onUnsave }: ProductCardProps) {
    ...
    return (
    <Card className="overflow-hidden group transition-all duration-300 ease-in-out bg-gradient-to-br from-primary/[0.02] via-background to-blue-500/[0.01] border border-[var(--border)] shadow-sm hover:shadow-lg hover:shadow-[var(--gold)]/10 flex flex-col relative h-full">
        {/* Subtle Decorative Blue Auras */}
        <div className="absolute -top-16 -left-16 w-48 h-48 bg-primary/5 rounded-full blur-3xl transition-all duration-700 opacity-30 group-hover:opacity-50 group-hover:scale-125 z-0" />
        ...
        <div className="relative z-10 flex flex-col h-full">
            <div className="p-3">
                <Link href={`/products/${product.id}`} className="block relative aspect-square overflow-hidden rounded-2xl bg-primary/[0.02]">
                    {hasImage ? (
                        <Image
